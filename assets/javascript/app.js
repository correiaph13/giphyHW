...
$("button").on("click", function () {
  ...
  $.ajax({
    ...
    for (var j = 0; j < results.length; j++) {
      ...
      var showImageStill = $("<img>");
      showImageStill.attr("src", results[j].images.fixed_height_still.url);

      console.log(results[j].images);

      gifDiv.append(p);
      gifDiv.append(showImage);
      gifDiv.append(showImageStill);
      showImage.addClass("movingImage");
      showImageStill.addClass("stillImage");
